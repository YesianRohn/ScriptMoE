# infer_deepseekocr.py - DeepSeek-OCR-2, LMDB -> JSONL
import os
import json
import lmdb
import cv2
...
def load_model(model_name="deepseek-ai/DeepSeek-OCR-2"):
...
def dump_one_lmdb_to_jsonl(
    ...
    tokenizer,
    out_jsonl: str,
    batch_size: int = 1,          # DeepSeek-OCR processes images one by one
):
    ...
    # DeepSeek-OCR requires image file paths, so create a temporary directory
    tmp_dir = tempfile.mkdtemp(prefix="deepseekocr_")

    with open(out_jsonl, "w", encoding="utf-8") as fw:
        with env.begin(write=False) as txn:
            for idx in tqdm(range(1, num_samples + 1), desc=f"DeepSeek-OCR | {dataset_name}"):
                ...
                # Save temporary image (DeepSeek-OCR needs a file path)
                ...
                result_file = os.path.join(result_dir, "result.mmd")
