"""Vendored from CC-OCR evaluation/evaluator/common.py (MIT)."""
...
def pick_response_text(json_path):
    ...
    if model_name.startswith("gpt") or model_name.startswith("o1"):
        response_text = (model_response.get("data", {}).get("response", {})
                         .get("choices", [{}])[0].get("message", {}).get("content", None))
    elif model_name.startswith("local_") or isinstance(model_response, str):
        response_text = model_response
    else:
        if model_name.startswith("claude"):
            content_list = model_response.get("content", None)
        elif model_name.startswith("gemini"):
            content_list = (model_response.get("candidates", [{}])[0]
                            .get("content", {}).get("parts", None))
        elif model_name.startswith("qwen"):
            content_list = (model_response.get("output", {}).get("choices", [{}])[0]
                            .get("message", {}).get("content", None))
        else:
            raise NotImplementedError(
                "The pick_response_text NOT implemented for model: {}".format(model_name))
...
class BaseMetric(object):
    ...
    def __call__(self, pdt_res_dir, gt_info, with_response_ratio=True, **kwargs):
        if isinstance(pdt_res_dir, dict):
            ...
            return ValueError(
                "invalid input: response dict or folder are required, "
                "but got {}".format(pdt_res_dir))
...
def summary(index_path, exp_dir_base, is_weighted_sum=False):
    ...
    for item in data_list:
        if not item.get("release", True):
            continue
        all_data_info[item["dataset"]] = item
    dataset_list = list(all_data_info.keys())
    return summary_multi_exp(exp_dir_base, dataset_list, is_weighted_sum=is_weighted_sum)
...
def summary_multi_exp(exp_dir_base, dataset_list=None, is_weighted_sum=False):
    ...
    for exp_name in os.listdir(exp_dir_base):
        ...
        for data_name in dataset_list:
            ...
            summary_info = (all_status_info.get(data_name, {})
                            .get("evaluation", {}).get("summary", {}))
            for metric_name, metric_value in summary_info.items():
                all_evaluate_info.setdefault(metric_name, {}) \
                    .setdefault(exp_name, {})[data_name] = (metric_value, total_num)
