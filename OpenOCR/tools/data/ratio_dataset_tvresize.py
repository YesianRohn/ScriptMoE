...
class MLTRatioDataSetTVResize(Dataset):
    """Multilingual (MLT) variant of RatioDataSetTVResize.

    Compared with the base RatioDataSetTVResize, this class adds:
    1. NFKC normalization on labels.
    2. Automatic rotation of vertical images (h > 1.5 * w) so that vertical
       text is treated as horizontal for both ratio computation and resize.
    3. Pre-built ratio bucket index for O(1) resampling instead of scanning
       the whole wh_ratio array on every retry.
    4. An extra ``gen_ratio`` field written into each sample.

    It is registered as an independent dataset name so that existing configs
    relying on RatioDataSetTVResize are not affected.
    """

    def __init__(self, config, mode, logger, seed=None, epoch=1, task='rec'):
        super(MLTRatioDataSetTVResize, self).__init__()
        self.ds_width = config[mode]['dataset'].get('ds_width', True)
        global_config = config['Global']
        dataset_config = config[mode]['dataset']
        loader_config = config[mode]['loader']
        ...
        for i in range(max_ratio + 1):
            logger.info((1 * (self.wh_ratio == i)).sum())
        self.wh_ratio_sort = np.argsort(self.wh_ratio)
        # 预先按 ratio 分桶建立索引，避免每次重抽时 O(N) 扫描整个 wh_ratio 数组
        self.ratio_to_ids = self._build_ratio_index(self.wh_ratio)
        self.ops = create_operators(dataset_config['transforms'],
                                    global_config)
    ...
    @staticmethod
    def _build_ratio_index(wh_ratio):
        """按 ratio 分桶建立索引字典，O(N) 一次构建，后续每次取桶 O(1)。"""
        ratio_to_ids = {}
        # 用 numpy 一次性按值分组，避免多次 np.where 全数组扫描
        wh_ratio_int = wh_ratio.astype(np.int64)
        unique_ratios = np.unique(wh_ratio_int)
        for r in unique_ratios:
            ids = np.where(wh_ratio_int == r)[0].tolist()
            ratio_to_ids[int(r)] = ids
            # 同时支持以 float 形式查询（与 wh_ratio dtype 一致）
            ratio_to_ids[float(r)] = ids
        return ratio_to_ids

    def _sample_id_by_ratio(self, ratio):
        """按 ratio 随机采样一个样本索引，O(1) 桶访问 + O(1) 采样。"""
        ratio_ids = self.ratio_to_ids.get(ratio)
        if ratio_ids is None:
            ratio_ids = self.ratio_to_ids.get(int(ratio))
        if ratio_ids is None or len(ratio_ids) == 0:
            # 兜底：理论上不会触发，保持与旧逻辑等价
            ratio_ids = np.where(self.wh_ratio == ratio)[0].tolist()
        return random.choice(ratio_ids)
    ...
    def get_wh_ratio(self):
        ...
        for idx in range(self.data_idx_order_list.shape[0]):
            ...
            if wh is None:
                ...
                w, h = wh.split('_')
            w, h = float(w), float(h)
            # 如果高大于宽的1.5倍，视为竖向图，按旋转后的尺寸计算ratio
            if h > w * 1.5:
                w, h = h, w
            wh_ratio.append(w / h)
        return wh_ratio
    ...
    def __getitem__(self, properties):
        ...
        sample_info = self.get_lmdb_sample_info(
            self.lmdb_sets[lmdb_idx]['txn'], file_idx)
        if sample_info is None:
            new_idx = self._sample_id_by_ratio(ratio)
            return self.__getitem__([img_width, img_height, new_idx, ratio])
        img, label = sample_info
        data = {'image': img, 'label': label}
        outs = transform(data, self.ops[:-1])
        if outs is not None:
